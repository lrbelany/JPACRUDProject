...
public class DropShipController {
	...
	@RequestMapping(path = "deleteid.do", method = RequestMethod.GET)
	public String deleteId(@RequestParam("id") int id, Model model) {
		if (dao.deleteId(id)) {
			model.addAttribute("kia", "Dropship KIA!");
		} else {
			model.addAttribute("kia", "not detected on radar");
		}
